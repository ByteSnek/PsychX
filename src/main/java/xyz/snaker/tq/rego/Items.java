...
import xyz.snaker.hiss.utility.Suppliers;
...
import xyz.snaker.tq.level.item.*;
...
public class Items
{
    ...
    public static final Supplier<Item> COMA_CRYSTAL = register("coma_crystal", ComaCrystalItem::new);
    public static final Supplier<Item> COMA_CRYSTAL_SWORD = register("coma_crystal_sword", ComaCrystalSword::new);
    //public static final Supplier<Item> DEBUG_TOOL = registerDebug("debug_tool", DebugTool::new);
    ...
    static <T extends Item> Supplier<T> registerConditional(String name, Supplier<T> item, Predicate<Object> condition)
    {
        if (!condition.test(null)) {
            SnakerLib.DEVLOGGER.infof("Ignore registry for item []:[]", Tourniqueted.MODID, name);
            return Sneaky.cast(Suppliers.EMPTY);
        }

        return register(name, item);
    }

    static <T extends Item> Supplier<T> registerDebug(String name, Supplier<T> item)
    {
        return registerConditional(name, item, Items::onlyForDebug);
    }
    ...
    static boolean onlyForDebug(Object obj)
    {
