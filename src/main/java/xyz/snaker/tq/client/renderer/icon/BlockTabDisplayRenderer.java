 ...
 **/
public class BlockTabDisplayRenderer extends CyclicalIconRenderer
{
    static final Logger LOGGER = Loggers.getLogger();

    static ItemStack blockToRender;

    public BlockTabDisplayRenderer()
    {
        ...
        if (level != null) {
            ...
            try {
                if (!block.equals(fallback)) {
                    ...
                    LOGGER.error("Could not render block tab icon! Ignoring and rendering fallback");
                    renderBlock(stack, block, scale, packedLight, packedOverlay);
                }
            } catch (Exception e) {
                if (notifyError) {
                    ...
                    LOGGER.errorf("Could not render []: []", className, errorMessage);
                    notifyError = false;
                }
            }
        }
