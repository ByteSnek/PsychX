...
{
    static Logger LOGGER = Loggers.getLogger();

    static ItemStack stackToRender;
        ...
        if (level != null) {
            ...
            try {
                if (!item.equals(fallback)) {
                    ...
                    LOGGER.error("Could not render item tab icon! Ignoring and rendering fallback");
                    renderStack(stack, item, scale, packedLight, packedOverlay);
                }
            } catch (Exception e) {
                if (notifyError) {
                    ...
                    LOGGER.warnf("Could not render []: []", className, errorMessage);
                    notifyError = false;
                }
            }
        }
