 ...
 **/
public class ObsidianFlatLevelPreset
{
    ...
    public static FlatLevelGeneratorPreset create(BootstapContext<FlatLevelGeneratorPreset> context)
    {
        ...
        list.addAll(FlatLayer.of(5, Blocks.OBSIDIAN), FlatLayer.of(Blocks.BEDROCK));

        for (int i = list.size() - 1; i >= 0; i--) {
            infos.add(list.getInfo(i));
        }

        return new FlatLevelGeneratorPreset(Items.OBSIDIAN.builtInRegistryHolder(), settings);
    }
}
