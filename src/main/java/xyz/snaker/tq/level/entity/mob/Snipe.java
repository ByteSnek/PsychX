        ...
        public void tick()
        {
            ...
            if (target != null) {
                ...
                if (target.distanceToSqr(snipe) < 4096 && snipe.hasLineOfSight(target)) {
                    ...
                    double distX = target.getX() - snipe.getX();
                    double distY = target.getY() - snipe.getY();
                    double distZ = target.getZ() - snipe.getZ();

                    snipe.setXRot(Maths.rotateTowards(distY, (distX * distX + distZ * distZ)));
                    snipe.xRotO = snipe.getXRot();

                    if (snipe.tickCount % delay == 0) {
                        CosmicRay ray = new CosmicRay(level, Snipe.this, distX, distY, distZ);

                        ray.shoot(distX, distY, distZ, velocity, inaccuracy);

                        level.addFreshEntity(ray);
                        level.playSound(null, target.getX(), target.getY(), target.getZ(), Sounds.SNIPE_SHOOT.get(), SoundSource.BLOCKS, 0.5F, (random.nextFloat() - random.nextFloat()) * 0.5F + 1);
                    }
                }
            }
        }
